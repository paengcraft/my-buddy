...
11. Click `Test Relay` and confirm `Health OK`.
12. Click the diagnostics button and paste the clipboard into a text editor; confirm it contains app, bundle, size, peer count, relay status, health, relay code, last error, and worker URL.
...
4. Turn off internet briefly on one Mac, then reconnect.
5. Confirm the app reports reconnecting and resumes after the connection returns.
